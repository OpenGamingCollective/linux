...
static int ayaneo_send_config(struct ayaneo *aya, u8 eject)
{
	...
	*cfg = template;
	...
	if (aya->rainbow) {
		/*
		 * HHD's AYANEO implementation uses HSV (275, 100, brightness)
		 * as the native rainbow amplitude. Colour and tempo are then
		 * generated by firmware; retain the user's static RGB cache.
		 */
...
/*
 * Native effects use separate attributes, as on other handheld HID LED
 * drivers. Do not overload hw_pattern's brightness/time pairs with modes.
 */
static const char * const ayaneo_effect_names[] = { "none", "rainbow" };

static ssize_t effect_index_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return sysfs_emit(buf, "none rainbow\n");
}
			   ...
			   char *buf)
{
	...
	guard(mutex)(&aya->lock);
	return sysfs_emit(buf, "%s\n", ayaneo_effect_names[aya->rainbow]);
}
			    ...
			    const char *buf, size_t count)
{
	...
	effect = sysfs_match_string(ayaneo_effect_names, buf);
	if (effect < 0)
		return effect;
	...
	if (effect && cdev->trigger)
		return -EBUSY;
#endif
	/* Apply earlier brightness writes before selecting a hardware effect. */
	flush_work(&cdev->set_brightness_work);

	guard(mutex)(&aya->lock);
	previous = aya->rainbow;
	if (previous == !!effect)
		return count;
	aya->rainbow = !!effect;
	ret = ayaneo_send_config(aya, 0);
	if (ret) {
		aya->rainbow = previous;
		return ret;
	}
	return count;
}
...
static int ayaneo_register_led(struct ayaneo *aya)
{
	...
	/* The multicolor class installs its own groups during registration. */
